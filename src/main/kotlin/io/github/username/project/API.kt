...
class APIImpl : API() {
    override fun getBiome(location: Location): NamespacedKey {
        ...
        val chunk = (location.chunk.getChunkSnapshot(true, true, false) as CraftChunkSnapshot)
        ...
        //从容器中提取生物群系对象，做一些计算(计算方法由bukkit提供)

        val resourceKey = biomeRegistry?.getResourceKey(biome)?.get()
        //一个存储资源的键的对象，包括命名空间 ID 和位置(?)

        val key = CraftNamespacedKey.fromMinecraft(resourceKey?.location())
